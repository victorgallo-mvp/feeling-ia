...
const { expirarPendentes } = require('./gerar');
...
async function estadoDe(doc) {
  if (doc.estado === 'gerando' || doc.estado === 'erro') return doc.estado;
...
router.get('/clientes/:id/documentos', async (req, res) => {
  try {
    await expirarPendentes(req.params.id);
    const { rows } = await pool.query(
      `SELECT id, tipo, criado_em, caminho, (markdown IS NOT NULL) AS tem_markdown, estado, erro,
    ...
    const docs = await Promise.all(rows.map(async ({ caminho, tem_markdown, estado, ...d }) => ({
      ...comLinks(d), estado: await estadoDe({ caminho, tem_markdown, estado }),
    })));
    res.json(docs);
  } catch (e) {
    console.error(e);
...
router.get('/documentos/:id/download', async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, cliente_id, cliente_nome, tipo, caminho, criado_em, markdown, estado FROM documentos_gerados WHERE id = $1',
      [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ erro: 'documento não encontrado' });
    const doc = rows[0];

    if (doc.estado === 'gerando') return res.status(409).json({ erro: 'documento ainda está sendo gerado' });
    if (doc.estado === 'erro') return res.status(409).json({ erro: 'a geração falhou — gere de novo' });
...
router.delete('/clientes/:id/documentos/perdidos', async (req, res) => {
  try {
    const { rows } = await pool.query(
      "SELECT id, caminho FROM documentos_gerados WHERE cliente_id = $1 AND markdown IS NULL AND estado <> 'gerando'", [req.params.id]
    );
    const perdidos = [];
