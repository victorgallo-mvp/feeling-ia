...
Cada problema sai como um **achado** com pilar, impacto, gravidade e facilidade (1–5), que ordenam os gargalos e o plano 30/60/90. O documento traz a matriz **problema → o que custa → o que dá para fazer → serviço Feeling**, restrita aos serviços da casa. O PDF de referência **não** é lido do cérebro na hora de escrever: ele é um diagnóstico preenchido de outro negócio e contaminaria os números — a estrutura está no prompt.

**Como o documento é escrito.** A primeira versão etiquetava cada parágrafo com FATO, OBSERVAÇÃO, INFERÊNCIA ou RECOMENDAÇÃO — 33 etiquetas num documento de 18 mil caracteres, o que lia como laudo pericial. A honestidade agora está na escrita, não em rótulo: o dado entra como frase normal com o número dentro (o número é a evidência), a consequência prática também, e o que é hipótese fecha a seção numa única linha *Nossa leitura:* — uma por pilar, com o pedido de confirmação ali, sem repetir o hedge. Cada pilar leva a nota no título (`## 4. Google e SEO local · 51/100 (2,6 de 5)`) e uma linha **Em uma frase:** para quem bate o olho, com parágrafos de 2 a 4 linhas e negrito só em número e nome próprio.
